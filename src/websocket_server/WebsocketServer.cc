  ...
  switch (_reason)
  {
    case LWS_CALLBACK_HTTP:
    {
      ...
      if (strcmp(URI, "/metrics") == 0)
      {
        ...
        unsigned char *buf = new unsigned char[buflen + LWS_PRE];
        int n;
...
/////////////////////////////////////////////////
bool WebsocketServer::Load(const WebsocketServer::Config &_config)//const tinyxml2::XMLElement *_elem)
{
  ...
  */
  this->publishPeriod = std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::duration<double>(1.0 / _config.publicationHz));
  ...
  this->authorizationKey = _config.authorizationKey;
  ...
  this->adminAuthorizationKey = _config.adminAuthorizationKey;
  ...
  */
  ...
  this->maxConnections = _config.maxConnections;
  ...
  this->queueSizePerConnection = _config.queueSizePerConnection;
  ...
  std::string sslCertFile = _config.sslCertFile;
  std::string sslPrivateKeyFile = _config.sslPrivateKeyFile;
  ...
  this->protocols.push_back(
    {
      ...
      this,
      // TX packet size restrictions
      0
    });

  // The terminator
  this->protocols.push_back({NULL, NULL, 0, 0, 0, 0, 0});
    ...
    const char *_data, const size_t _size)
{
  if (_connection)
  {
    ...
    if (static_cast<int>(_connection->buffer.size()) <
...
//////////////////////////////////////////////////
void WebsocketServer::RunNonBlocking()
{
  this->run = true;
